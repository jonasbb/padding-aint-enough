...
pub struct ConstantRate<S, T, DUMMY>
where
    S: Stream<Item = T>,
    DUMMY: Fn() -> T,
{
...
impl<S, T, DUMMY> ConstantRate<S, T, DUMMY>
where
    S: Stream<Item = T>,
    DUMMY: Fn() -> T,
{
    pub fn new(interval: Duration, stream: S, make_dummy: DUMMY) -> Self {
        Self {
...
impl<S, T, DUMMY> Stream for ConstantRate<S, T, DUMMY>
where
    S: Stream<Item = T>,
    S::Error: Into<Error>,
    DUMMY: Fn() -> T,
        ...
        match self.interval.poll()? {
            Async::Ready(Some(_)) => {
                ...
                match self.stream.poll() {
                    Ok(x) => match x {
                        Async::Ready(Some(t)) => Ok(Async::Ready(Some(t))),
                        Async::Ready(None) => Ok(Async::Ready(None)),
                        Async::NotReady => {
                            // No packet to send, send dummy
                            Ok(Async::Ready(Some((self.make_dummy)())))
                        }
                    },
...
mod tests {
    ...
    fn test_constant_time_ensure_time_gap() {
        ...
        let cr = ConstantRate::new(dur, items, || 99u8);
    ...
    fn test_constant_time_insert_dummy() {
        ...
        let cr_slow = ConstantRate::new(dur_long, items, || 255u8);
        let cr = ConstantRate::new(dur_short, cr_slow, move || dummy);
