...
#         continue
    ...
    for ext in [".json.xz.log", ".json.xz.cr.log"]:
        try:
            res = dict(parse_log_data(f"/home/jbushart/results/{name}{ext}")[0])["PluralityThenMinDist"]
            break
        except Exception:
            pass
    else:
#         raise OSError(2, "No such file or directory", name)
        res = [100] + ([1] * 10)
    print(name, res)
    print("   ", "5/10 (%):", res[5]/res[0] *100, "Query Overhead (%):", (y-1)*100, "Time Overhead (%):", (x-1)*100)
    # X / 10 correct
    frac = res[5] / res[0] * 10
...
#     frac = 1/(x+y) if x+y>0 else 1
    ...
    s = np.abs(ab).max()
    ax.scatter([x], [y], marker=full_circle, s=s * circle_size, facecolor=adjust_hue_grey(name, grey), alpha=0.2, zorder=10, linewidths=0)
    color = adjust_hue(name, name2color(name))
    ax.scatter([x], [y], marker=ab, s=s * circle_size, facecolor=color, label=name, alpha=0.5, zorder=100, linewidths=0)
    
    if "-0.9p" in name:
        name = name[:-len("-0.9p")]
        handle = ax.scatter([-1], [-1], marker=full_circle, s=s * 200, facecolor=color, label=name, linewidths=0)
        legends.append((handle, name2name[name]))
    
for ax in axes:
    ax.set_ylim(bottom=1, top=3.5)
axes[0].set_xlim(left=1 -.0025, right=1.01)
...
axes[0].arrow(1.00625, 2.7, 0, 1.3-2.7, length_includes_head=True, color="black", width=0.00001, head_width=0.0005, head_length=0.1)
...
ax.set_axis_off()
fig.legend([x[0] for x in legends], [x[1] for x in legends], ncol=5, loc='center', frameon=False)
fig.tight_layout()
